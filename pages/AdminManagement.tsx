...
import { Student, UserType, ContactMessage } from '../types';
import {
   Users,
   GraduationCap,
   ArrowLeft,
...
const AdminManagement: React.FC = () => {
   ...
   const [activeTab, setActiveTab] = useState<'list' | 'register' | 'messages'>('list');
   ...
   const loadData = async () => {
      ...
      try {
         ...
         const messagesData = await dataService.getContactMessages();
         ...
         <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl">
               ...
               <button
                  ...
                  className={`px-6 py-2 rounded-lg text-sm font-black transition-all ${activeTab === 'messages' ? 'bg-white text-brand-dark shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
               >
                  Mensagens
               </button>
            </div>
            ...
            {isLoadingData ? (
               ...
               <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                     ...
                     <div className="flex items-center gap-2 text-[10px] font-black uppercase text-gray-400">
                        <MessageCircle size={14} /> Total: {messages.length} Mensagens
                     </div>
                  </div>

                  <div className="grid gap-4">
                     {filteredMessages.map(msg => (
                        <div key={msg.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
                           <div className="flex flex-col md:flex-row gap-6">
                              <div className="md:w-1/4 space-y-3">
                                 <div className="flex items-center gap-2">
                                    <div className="w-10 h-10 bg-brand-light/20 rounded-full flex items-center justify-center text-brand-dark font-black">
                                       {msg.name[0]}
                                    </div>
                                    <div>
                                       <p className="font-black text-gray-900 leading-tight">{msg.name}</p>
                                 ...
                                 {msg.interest && (
                                    <span className="block text-xs font-bold text-brand-dark bg-brand-light/10 px-3 py-1 rounded-full w-fit">
                                       {msg.interest}
                                    </span>
                                 )}
                              </div>
                              <div className="md:w-3/4 bg-gray-50/50 p-4 rounded-xl border border-gray-50">
                     ...
                     {filteredMessages.length === 0 && (
                        <div className="py-20 text-center">
                           <p className="text-gray-400 font-black uppercase tracking-widest text-xs">Nenhuma mensagem encontrada.</p>
                        </div>
                     )}
                  </div>
               </div>
